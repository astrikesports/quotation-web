...
export default function OrderStatusPage() {
  ...
  const bookingPendingOrders = orders.filter(
    (o) => o.status === "booking pending"
  );
  ...
  const updateCOD = async (id, cod_amount) => {
    const { error } = await supabase

      .from("order_status")

      .update({
        cod_amount: cod_amount ? Number(cod_amount) : null,
      })
  ...
  const updateAWB = async (id, awb_link) => {
    await supabase
  ...
  const uploadBill = async (e, order) => {
    const file = e.target.files[0];

    if (!file) return;

    const fileName = `bill-${Date.now()}-${file.name}`;
    ...
    await supabase
      ...
      .update({
        bill_image: publicUrlData.publicUrl,
      })
  ...
  const uploadQuotationImage = async (e, order) => {
    const file = e.target.files[0];

    if (!file) return;

    const fileName = `quotation-${Date.now()}-${file.name}`;
    ...
    await supabase
      ...
      .update({
        quotation_image: publicUrlData.publicUrl,
      })
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <button
          onClick={() => {
            ...
            setActiveTab(activeTab === "confirmed" ? "all" : "confirmed");
          }}
        ...
        <button
          onClick={() => {
            ...
            setActiveTab(activeTab === "preparing" ? "all" : "preparing");
          }}
          className={`rounded-[32px] p-8 text-left shadow-2xl border-[4px] transition-all duration-200 hover:scale-[1.02] ${
            activeTab === "preparing"
        ...
        <button
          onClick={() => {
            ...
            setActiveTab(
              activeTab === "booking pending" ? "all" : "booking pending"
            );
          }}
          className={`rounded-[32px] p-8 text-left shadow-2xl border-[4px] transition-all duration-200 hover:scale-[1.02] ${
        ...
        <button
          onClick={() => {
            ...
            setActiveTab(activeTab === "shipped" ? "all" : "shipped");
          }}
          className={`rounded-[32px] p-8 text-left shadow-2xl border-[4px] transition-all duration-200 hover:scale-[1.02] ${
            activeTab === "shipped"
              ...
              {paginatedOrders.map((order) => (
                ...
                >
                  ...
                  <td className="px-4 py-5">
                    <input
                      type="number"
                      defaultValue={order.cod_amount || 0}
                      onBlur={(e) => updateCOD(order.id, e.target.value)}
                      placeholder="COD"
                  ...
                  <td className="px-4 py-5">
                    <input
                      type="text"
                      defaultValue={order.awb_link || ""}
                      onBlur={(e) => updateAWB(order.id, e.target.value)}
                    ...
                    <div className="flex items-center gap-2">
                      <label className="w-11 h-11 rounded-2xl bg-black text-white flex items-center justify-center cursor-pointer shrink-0">
                        ...
                        <input
                          ...
                          accept="image/*,.pdf"
                          hidden
                          onChange={(e) => uploadBill(e, order)}
                        />
                      </label>

                      {order.bill_image && (
                        <a
                          href={order.bill_image}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <div className="flex items-center gap-2">
                      <label className="w-11 h-11 rounded-2xl bg-green-500 text-black flex items-center justify-center cursor-pointer shrink-0">
                        ...
                        <input
                          ...
                          accept="image/*"
                          hidden
                          onChange={(e) => uploadQuotationImage(e, order)}
                        />
                      </label>

                      {order.quotation_image && (
                        <img
                          src={order.quotation_image}
                          alt="quotation"
